-- Run once before enabling foreign-currency entry. Existing amounts are USD;
-- this nullable column does not recalculate or change any existing expense.
BEGIN;

ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS currency_conversion JSONB;
...
BEGIN
  ...
  INSERT INTO transactions (trip_id, description, total_amount, payer_id, split_type, receipt_url, line_items, status, currency_conversion)
  VALUES (selected_trip, expense->>'description', usd_total, (expense->>'payer_id')::uuid,
          expense->>'split_type', expense->>'receipt_url', NULLIF(expense->'line_items', 'null'::jsonb), 'finalized', conversion)
