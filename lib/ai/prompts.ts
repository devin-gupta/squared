...
${shape}
${currencyRules}
Use a clear description of what was purchased. When no currency is specified, use the supplied trip default currency, or USD if there is no trip default. Preserve any explicit payer and split instructions; do not guess who paid. Match only exact or clearly unambiguous available member names. A shorter name is unambiguous when it occurs in exactly one available member name; return that member's full exact name. Default to equal split. Custom adjustments are absolute nonnegative amounts owed, must include every participant (zero for excluded members), and sum to total_amount. If a total cannot be determined, use total_amount: 0 for review. Only include amount_text when copying one actual amount from the input; omit it when calculating a total from several amounts.
