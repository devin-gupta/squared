...
> {
  ...
  if (userId) {
    const { data: memberTrips, error: membershipError } = await supabase
      .from("trip_members")
      .select("trip_id")
      .eq("user_id", userId);
    ...
    if (membershipError)
      throw new Error(
        "We couldn’t load your trips. Check your connection and try again.",
      );
    if (memberTrips && memberTrips.length > 0) {
      const tripIds = (memberTrips as Array<{ trip_id: string }>).map(
        (m) => m.trip_id,
      );
  ...
  const { data: trips, error } = await query.order("created_at", {
    ascending: false,
  });
